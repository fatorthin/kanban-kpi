<div x-data="{ open: false }" class="relative" wire:poll.10s>
    ...
    {{-- Dropdown --}}
    <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" class="notification-dropdown" style="display: none;">
        ...
        <div class="notification-header">
            ...
            <div style="display:flex;align-items:center;gap:8px;">
                ...
                @if ($unreadCount > 0)
                    <button type="button" class="btn btn-ghost btn-sm" wire:click="markAllAsRead">
                        Tandai semua
                    </button>
                @endif
            </div>
        ...
        <div class="notification-list">
            @forelse($activities as $activity)
                <div class="notification-item {{ $activity->is_read ? '' : 'unread' }}" style="display:flex; gap:12px; padding:12px 16px; border-bottom:1px solid var(--color-border);">
                    ...
                    <div style="flex:1; min-width:0;">
                        ...
                        @if (!$activity->is_read)
                            <div style="margin-top:8px;">
                                <button type="button" class="btn btn-ghost btn-sm" wire:click="markAsRead({{ $activity->id }})">
                                    Tandai sudah dibaca
                                </button>
                            </div>
                        @endif
